...
public class LexAn implements AutoCloseable {
	...
	private void nextToken() {
		...
		// INTCONST with digit separators - CORRECTED VERSION
		if (isDigit(startChar)) {
			nextChar();
			
			// Read the integer with potential digit separators
			while (isDigit(buffChar) || buffChar == '.') {
				if (buffChar == '.') {
					// We found a potential digit separator
					nextChar(); // consume the '.'
					
					// Count how many digits follow (we need exactly 3)
					int digitCount = 0;
					
					// First, count ALL consecutive digits to validate the group size
					...
					// Validate: must be exactly 3 digits, no more, no less
					if (digitCount != 3) {
						if (digitCount < 3) {
							throw new Report.Error(
								new Report.Location(startLine, startCol),
								"Digit separator must be followed by exactly 3 digits, found " + digitCount
							);
						} else {
							throw new Report.Error(
								new Report.Location(startLine, startCol),
								"Too many digits after separator: found " + digitCount + ", expected 3"
							);
						}
					}
					
					// Continue to check for more groups or end of number
				} else {
					// Regular digit
					sb.append((char) buffChar);
					nextChar();
			...
			// Additional validation for the overall format
			String fullNumber = sb.toString();
			
			// Check for leading zeros in the entire number
			if (fullNumber.length() > 1 && fullNumber.charAt(0) == '0') {
				throw new Report.Error(
					new Report.Location(startLine, startCol),
