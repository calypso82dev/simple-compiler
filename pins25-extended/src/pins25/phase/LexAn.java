...
public class LexAn implements AutoCloseable {
    ...
    private static final Map<String, Token.Symbol> KEYWORDS = Map.ofEntries(
            ...
            Map.entry("until", Token.Symbol.UNTIL),
            Map.entry("break", Token.Symbol.BREAK),
    ...
    private void skipSingleLineComment() {
        ...
        while (true) {
            ...
            while (buffChar != EOF && !isEndOfLine(buffChar)) {
                lineContent.append((char) buffChar);
                nextChar();
            }
            ...
            // If doesn't continue with --, stop here
            if (!continues) {
                break;
            }

            // Otherwise, treat next line as comment content
