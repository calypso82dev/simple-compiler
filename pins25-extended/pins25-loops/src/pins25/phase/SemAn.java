...
public class SemAn {
	...
	private static class TypeResolver {
		...
		private class ResolverVisitor implements AST.FullVisitor<Object, ResolverVisitor.Pass> {
            ...
            @Override
            public Object visit(final AST.WhileStmt whileStmt, final Pass pass) {
                // Increse loop depth
                loopDepth++;
                // Visit statements (BREAK valid)
                whileStmt.stmts.accept(this, pass);
                // Decrese loop depth
                loopDepth--;
            ...
            @Override
            public Object visit(final AST.UntilStmt untilStmt, final Pass pass) {
                // Increse loop depth
                loopDepth++;
                // Visit statements (BREAK valid)
                untilStmt.stmts.accept(this, pass);
                // Decrese loop depth
                loopDepth--;
            ...
            @Override
            public Object visit(final AST.BreakStmt breakStmt, final Pass pass) {
                // Check if BREAK is in While or DoWhile loop (Depth > 0)
                if (loopDepth == 0) {
                    throw new Report.Error(attrAST.attrLoc.get(breakStmt),
