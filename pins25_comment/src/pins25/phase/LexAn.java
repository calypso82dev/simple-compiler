...
public class LexAn implements AutoCloseable {
	...
	/** Flag to indicate if the next line should be treated as a comment due to -- continuation */
	...
	private void nextToken() {
		...
		// Check if this line should be treated as a comment due to previous line ending with --
		if (nextLineIsComment) {
			isComment = true;
			nextLineIsComment = false; // Reset the flag for next time
		}
		...
		switch (startChar) {
			...
			// Comment handling (both // and -- continuation)
		if (isComment) {
			// Check if this line ends with -- (line continuation)
			boolean hasLineContinuation = false;
			
			// Read until new line, checking for -- at the end
			StringBuilder commentBuffer = new StringBuilder();
			while (!isEndOfLine(buffChar) && buffChar != -1) {
				commentBuffer.append((char) buffChar);
				nextChar();
			}
			
			// Check if line ends with --
			String commentLine = commentBuffer.toString().trim();
			if (commentLine.endsWith("--")) {
				hasLineContinuation = true;
			}
			
			// Set flag for next line
			nextLineIsComment = hasLineContinuation;
