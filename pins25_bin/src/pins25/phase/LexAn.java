...
public class LexAn implements AutoCloseable {
	...
	private void nextToken() {
		...
		if (isDigit(startChar)) {
			...
			if (sb.charAt(0) == '0' && (buffChar == 'b' || buffChar == 'B')) {
				// Binary number
				sb.append((char) buffChar); // Add 'b' or 'B'
				nextChar();
				...
				if (!isBinaryDigit(buffChar)) {
					throw new Report.Error(
						...
						"Invalid binary number: must contain at least one binary digit after 0b"
					);
				}
				
				// Read binary digits and convert to decimal
				StringBuilder binaryDigits = new StringBuilder();
				while (isBinaryDigit(buffChar)) {
					binaryDigits.append((char) buffChar);
					nextChar();
				}
				
				// Check if we stopped on an invalid character that looks like it should be part of the number
				if (isDigit(buffChar)) {
					throw new Report.Error(
						new Report.Location(startLine, startCol),
						"Invalid binary digit '" + (char) buffChar + "' in binary number"
					);
				}
				
				// Convert binary to decimal and replace the content
				try {
					int decimalValue = Integer.parseInt(binaryDigits.toString(), 2);
					sb.setLength(0); // Clear the buffer
					sb.append(decimalValue); // Store as decimal
				} catch (NumberFormatException e) {
					throw new Report.Error(
						new Report.Location(startLine, startCol),
						"Invalid binary number: " + binaryDigits.toString()
					);
				}
			} else {
		...
		// 	// Check for special number formats (binary, octal)
		// 	if (sb.charAt(0) == '0') {
		// 		if (buffChar == 'b' || buffChar == 'B') {
		// 			// Binary number
		// 			sb.append((char) buffChar); // Add 'b' or 'B'
		// 			nextChar();
		...
		// 				);
		// 			}
		// 		} else if (buffChar == 'o' || buffChar == 'O') {
		// 			// Octal number
		// 			sb.append((char) buffChar); // Add 'o' or 'O'
		// 			nextChar();
		...
		// 				throw new Report.Error(
		// 					new Report.Location(startLine, startCol),
		// 					"Invalid octal number: must contain at least one octal digit after 0o"
		// 				);
		// 			}
					
		// 			// Read octal digits and convert to decimal
		// 			StringBuilder octalDigits = new StringBuilder();
		// 			while (isOctalDigit(buffChar)) {
		// 				octalDigits.append((char) buffChar);
		// 				nextChar();
		// 			}
		...
		// 				throw new Report.Error(
		// 					new Report.Location(startLine, startCol),
		// 					"Invalid octal digit '" + (char) buffChar + "' in octal number"
		// 				);
		// 			}
					
		// 			// Convert octal to decimal and replace the content
		// 			try {
		// 				int decimalValue = Integer.parseInt(octalDigits.toString(), 8);
		// 				sb.setLength(0); // Clear the buffer
		// 				sb.append(decimalValue); // Store as decimal
		// 			} catch (NumberFormatException e) {
		// 				throw new Report.Error(
		// 					new Report.Location(startLine, startCol),
		// 					"Invalid octal number: " + octalDigits.toString()
		// 				);
		// 			}
		// 		} else {
		...
		// // Add these helper methods after your existing helper methods:
