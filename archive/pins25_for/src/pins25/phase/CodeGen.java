	...
	private static class CodeGenerator {
		...
		private class Generator implements AST.FullVisitor<List<PDM.CodeInstr>, Mem.Frame> {
			...
			if (varAccess instanceof Mem.AbsAccess absAccess) { 
					...
					varData.add(new PDM.LABEL(varDef.name, varLoc));           // Label "a"
					varData.add(new PDM.SIZE(absAccess.size, varLoc));         // Reserve space for variable
					
					// 2. SECOND: Initialization data (if exists)
					if (absAccess.inits != null) { 
						varData.add(new PDM.LABEL(varDef.name + "_init", varLoc)); // Label "a_init"
						...
						varInstr.add(new PDM.NAME(varDef.name, varLoc));           // Push variable address
						varInstr.add(new PDM.NAME(varDef.name + "_init", varLoc)); // Push init data address
			...
			@Override
			public List<PDM.CodeInstr> visit(AST.ForStmt forStmt, Mem.Frame frame) {
				List<PDM.CodeInstr> forInstr = new ArrayList<>();
				Report.Locatable forLoc = attrAST.attrLoc.get(forStmt);
				
				String startLabel = nextLabel();   // Beginning of loop
				String bodyLabel = nextLabel();    // Loop body  
				String endLabel = nextLabel();     // End of loop
			
				
				// Initialize loop variable: var = startExpr
				// Use the same pattern as regular variable assignments
				// First, create a simple VarExpr and AssignStmt but don't add to attrLoc
				AST.VarExpr loopVar = new AST.VarExpr(forStmt.varName);
				AST.AssignStmt initAssign = new AST.AssignStmt(loopVar, forStmt.startExpr);
				
				// Generate the assignment using existing visitor (without adding to attrLoc)
				List<PDM.CodeInstr> initCode = visit(initAssign, frame);
				if (initCode != null) {
					forInstr.addAll(initCode);
				}
				
				// Start label - condition check
				forInstr.add(new PDM.LABEL(startLabel, forLoc));
				
				// Check condition: loopVar <= endExpr
				AST.VarExpr loopVarForCondition = new AST.VarExpr(forStmt.varName);
				List<PDM.CodeInstr> varCode = visit(loopVarForCondition, frame);
				if (varCode != null) {
					forInstr.addAll(varCode);
				}
				
				List<PDM.CodeInstr> endCode = forStmt.endExpr.accept(this, frame);
				if (endCode != null) {
					forInstr.addAll(endCode);
				}
				
				// Check if var <= endExpr (continue if true, exit if false)
				forInstr.add(new PDM.OPER(PDM.OPER.Oper.LEQ, forLoc));
				
				// CJMP: if var <= endExpr (true), go to body; if false, exit
				forInstr.add(new PDM.NAME(bodyLabel, forLoc));   // addr2 - go to body (condition true)
				forInstr.add(new PDM.NAME(endLabel, forLoc));    // addr1 - exit (condition false)
				forInstr.add(new PDM.CJMP(forLoc));
				
				// Body label and statements
				forInstr.add(new PDM.LABEL(bodyLabel, forLoc));
				for (AST.Stmt stmt : forStmt.stmts) {
					List<PDM.CodeInstr> stmtCode = stmt.accept(this, frame);
					if (stmtCode != null) {
						forInstr.addAll(stmtCode);
					}
				}
				
				// Increment loop variable: var = var + 1
				AST.VarExpr loopVarForIncrement = new AST.VarExpr(forStmt.varName);
				AST.VarExpr loopVarValue = new AST.VarExpr(forStmt.varName);
				AST.AtomExpr oneExpr = new AST.AtomExpr(AST.AtomExpr.Type.INTCONST, "1");
				AST.BinExpr increment = new AST.BinExpr(AST.BinExpr.Oper.ADD, loopVarValue, oneExpr);
				AST.AssignStmt incAssign = new AST.AssignStmt(loopVarForIncrement, increment);
				
				List<PDM.CodeInstr> incCode = visit(incAssign, frame);
				if (incCode != null) {
					forInstr.addAll(incCode);
				}
				
				// Jump back to condition check
				forInstr.add(new PDM.NAME(startLabel, forLoc));
				forInstr.add(new PDM.UJMP(forLoc));
				
				// End label
				forInstr.add(new PDM.LABEL(endLabel, forLoc));
			...
			public List<PDM.CodeInstr> visit(AST.AtomExpr atomExpr, Mem.Frame frame) {
				...
				switch (atomExpr.type) {
					...
					case STRCONST -> {
						...
						// Check if we're currently processing a function call
						if (processingCallExpr) {
							...
							// Generate INIT format with metadata for variable initialization
							String initLabel = "STR_INIT_" + labelCounter++;
							atomInstr.add(new PDM.NAME(initLabel, atomLoc));
							
							List<PDM.DataInstr> initData = new ArrayList<>();
							initData.add(new PDM.LABEL(initLabel, atomLoc));
							// INIT format: n, n1, l1, data...
							initData.add(new PDM.DATA(1, atomLoc));                      // n = 1 (one init group)
							initData.add(new PDM.DATA(1, atomLoc));                      // n1 = 1 (repeat 1 time)
							initData.add(new PDM.DATA(chars.size() + 1, atomLoc));       // l1 = length + null terminator
							for (Integer ch : chars) {
								initData.add(new PDM.DATA(ch, atomLoc));
							}
							attrAST.attrData.put(atomExpr, initData);
						}
					}
				}
			...
			// Add this field to the Generator class
			private boolean processingCallExpr = false;
			...
			public List<PDM.CodeInstr> visit(AST.CallExpr callExpr, Mem.Frame frame) {
				...
				if (calledFrame != null) {
					// Set flag before processing arguments
        			processingCallExpr = true;
					...
					// Reset flag after processing arguments
        			processingCallExpr = false;
