	...
	private AST.Stmt parseState()
	{
		...
		switch (token.symbol())
		{
			...
			case Token.Symbol.LET: // let defs2 in states end
				...
				this.attrLoc.put(stmt, new Report.Location(startLoc, endLoc));
				break;
			case Token.Symbol.FOR: // for id assign expr to expr do states end
				// Production: state -> for id assign expr to expr do states end
				startLoc = check(Token.Symbol.FOR); // consume 'for'
				Token forVar = check(Token.Symbol.IDENTIFIER, "Expected variable name after 'for'");
				check(Token.Symbol.ASSIGN, "Expected '=' after for variable");
				AST.Expr forStartExpr = parseExpr(); // Parse start expression
				check(Token.Symbol.TO, "Expected 'to' after start expression");
				AST.Expr forEndExpr = parseExpr(); // Parse end expression
				check(Token.Symbol.DO, "Expected 'do' after end expression");
				List<AST.Stmt> forStmts = parseStates(); // Parse statements
				endLoc = check(Token.Symbol.END, "Expected 'end' after for body");
				stmt = new AST.ForStmt(forVar.lexeme(), forStartExpr, forEndExpr, forStmts);
				this.attrLoc.put(stmt, new Report.Location(startLoc, endLoc));
				break;
