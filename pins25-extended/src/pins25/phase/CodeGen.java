	...
	private static class CodeGenerator {
		...
		private class Generator implements AST.FullVisitor<List<PDM.CodeInstr>, Mem.Frame> {
			...
			public List<PDM.CodeInstr> visit(AST.WhileStmt whileStmt, Mem.Frame frame) {
				...
				// Generate condition code (value is now on stack) - expr
                ...
                // Add end label on exitLabels for break statments
                ...
                // Remove end label from exitLabels
                loopExitLabels.pop();
            ...
            @Override
            public List<PDM.CodeInstr> visit(AST.UntilStmt untilStmt, Mem.Frame frame) {
                List<PDM.CodeInstr> codeInstr = new ArrayList<>();
                ...
                // 1. Body
                for (AST.Stmt stmt : untilStmt.stmts) {
                    List<PDM.CodeInstr> stmtCode = stmt.accept(this, frame);
                    codeInstr.addAll(stmtCode);
                }
                ...
                // Generate condition code (value is now on stack)
                List<PDM.CodeInstr> condCode = untilStmt.cond.accept(this, frame);
                codeInstr.addAll(condCode);

                // First jump: CJMP - if condition == 0, exit loop; if condition != 0, continue to body
                codeInstr.add(new PDM.NAME(startLabel, loc));   // addr2 - go to start (condition != 0)
                codeInstr.add(new PDM.NAME(endLabel, loc));    // addr1 - go to end (condition == 0) - EXIT LOOP
                codeInstr.add(new PDM.CJMP(loc));
            ...
            public List<PDM.CodeInstr> visit(AST.BreakStmt breakStmt, Mem.Frame frame) {
                List<PDM.CodeInstr> codeInstr = new ArrayList<>();
                Report.Locatable loc = attrAST.attrLoc.get(breakStmt);

                // Add current loop ExitLabel as Address
                codeInstr.add(new PDM.NAME(loopExitLabels.peek(), loc));
                codeInstr.add(new PDM.UJMP(loc));
