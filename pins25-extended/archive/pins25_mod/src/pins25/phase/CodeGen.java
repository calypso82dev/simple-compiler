	...
	private static class CodeGenerator {
		...
		private class Generator implements AST.FullVisitor<List<PDM.CodeInstr>, Mem.Frame> {
			...
			public List<PDM.CodeInstr> visit(AST.BinExpr binExpr, Mem.Frame frame) {
				...
				// Special handling for MOD operation
				if (binExpr.oper == AST.BinExpr.Oper.MOD) {
					// Implement a % b = a - (a / b) * b
					// Don't use the already generated code, regenerate everything
					
					// Clear any existing operand code
					binInstr.clear();
					
					// Generate: a - (a / b) * b
					
					// First: generate a
					List<PDM.CodeInstr> fstCode1 = binExpr.fstExpr.accept(this, frame);
					if (fstCode1 != null) {
						binInstr.addAll(fstCode1);
					}
					
					// Generate: (a / b) * b
					// Start with a / b
					List<PDM.CodeInstr> fstCode2 = binExpr.fstExpr.accept(this, frame);
					if (fstCode2 != null) {
						binInstr.addAll(fstCode2);
					}
					
					List<PDM.CodeInstr> sndCode1 = binExpr.sndExpr.accept(this, frame);
					if (sndCode1 != null) {
						binInstr.addAll(sndCode1);
					}
					
					// Stack: [a, a, b]
					// Compute a / b
					binInstr.add(new PDM.OPER(PDM.OPER.Oper.DIV, binLoc));
					
					// Stack: [a, quotient]
					// Now multiply by b
					List<PDM.CodeInstr> sndCode2 = binExpr.sndExpr.accept(this, frame);
					if (sndCode2 != null) {
						binInstr.addAll(sndCode2);
					}
					
					// Stack: [a, quotient, b]
					binInstr.add(new PDM.OPER(PDM.OPER.Oper.MUL, binLoc));
					
					// Stack: [a, product]
					// Finally: a - product
					binInstr.add(new PDM.OPER(PDM.OPER.Oper.SUB, binLoc));
					
					return binInstr;
				}

				// Apply binary operator for all other operations
				PDM.OPER.Oper oper = switch (binExpr.oper) {
					case OR -> PDM.OPER.Oper.OR;
					case AND -> PDM.OPER.Oper.AND;
					case EQU -> PDM.OPER.Oper.EQU;
					...
					case MUL -> PDM.OPER.Oper.MUL;
					case DIV -> PDM.OPER.Oper.DIV;
					case MOD -> throw new Report.Error("MOD should be handled above");
				};
