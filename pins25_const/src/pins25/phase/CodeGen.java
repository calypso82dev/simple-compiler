...
public class CodeGen {
	...
	private static class CodeGenerator {
		...
		public List<PDM.CodeInstr> visit(AST.BinExpr binExpr, Mem.Frame frame) {
			...
			// Check if both operands are constants - if so, fold them
			if (isConstantExpression(binExpr.fstExpr) && isConstantExpression(binExpr.sndExpr)) {
				// Evaluate both constants
				int leftValue = evaluateConstant(binExpr.fstExpr);
				int rightValue = evaluateConstant(binExpr.sndExpr);
				
				// Compute the result at compile time
				int result = computeConstantOperation(leftValue, rightValue, binExpr.oper);
				
				// Generate a single PUSH instruction with the computed result
				binInstr.add(new PDM.PUSH(result, binLoc));
				return binInstr;
			}

			// If not both constants, generate normal code
		...
		// Add these helper methods to your Generator class:

		// Helper method to check if an expression is a constant
		private boolean isConstantExpression(AST.Expr expr) {
			if (expr instanceof AST.AtomExpr atomExpr) {
				return atomExpr.type == AST.AtomExpr.Type.INTCONST;
			}
			if (expr instanceof AST.BinExpr binExpr) {
				return isConstantExpression(binExpr.fstExpr) && isConstantExpression(binExpr.sndExpr);
			}
			if (expr instanceof AST.UnExpr unExpr) {
				// Only arithmetic unary operators (not MEMADDR, VALUEAT)
				return (unExpr.oper == AST.UnExpr.Oper.ADD || 
						unExpr.oper == AST.UnExpr.Oper.SUB || 
						unExpr.oper == AST.UnExpr.Oper.NOT) && 
					isConstantExpression(unExpr.expr);
			}
			return false;
		}

		// Helper method to evaluate a constant expression
		private int evaluateConstant(AST.Expr expr) {
			if (expr instanceof AST.AtomExpr atomExpr && atomExpr.type == AST.AtomExpr.Type.INTCONST) {
				return Integer.parseInt(atomExpr.value);
			}
			if (expr instanceof AST.BinExpr binExpr) {
				int left = evaluateConstant(binExpr.fstExpr);
				int right = evaluateConstant(binExpr.sndExpr);
				return computeConstantOperation(left, right, binExpr.oper);
			}
			if (expr instanceof AST.UnExpr unExpr) {
				int operand = evaluateConstant(unExpr.expr);
				return switch (unExpr.oper) {
					case ADD -> operand;
					case SUB -> -operand;
					case NOT -> operand == 0 ? 1 : 0;
					default -> throw new Report.Error("Cannot evaluate non-arithmetic unary operator");
				};
			}
			throw new Report.Error("Cannot evaluate non-constant expression");
		}

		// Helper method to compute constant operations
		private int computeConstantOperation(int left, int right, AST.BinExpr.Oper oper) {
			return switch (oper) {
				case ADD -> left + right;
				case SUB -> left - right;
				case MUL -> left * right;
				case DIV -> {
					if (right == 0) {
						throw new Report.Error("Division by zero in constant expression");
					}
					yield left / right;
				}
				case MOD -> {
					if (right == 0) {
						throw new Report.Error("Modulo by zero in constant expression");
					}
					yield left % right;
				}
				case EQU -> left == right ? 1 : 0;
				case NEQ -> left != right ? 1 : 0;
				case GTH -> left > right ? 1 : 0;
				case LTH -> left < right ? 1 : 0;
