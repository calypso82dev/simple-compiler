...
public class CodeGen {
	...
	private static class CodeGenerator {
		...
		private class Generator implements AST.FullVisitor<List<PDM.CodeInstr>, Mem.Frame> {
			...
			@Override
			public List<PDM.CodeInstr> visit(AST.DoUntilStmt doUntilStmt, Mem.Frame frame) {
				List<PDM.CodeInstr> doUntilInstr = new ArrayList<>();
				Report.Locatable doUntilLoc = attrAST.attrLoc.get(doUntilStmt);
				
				String startLabel = nextLabel();    // Beginning of loop body
				String endLabel = nextLabel();      // End of loop
				
				// Start label - beginning of loop body
				doUntilInstr.add(new PDM.LABEL(startLabel, doUntilLoc));
				
				// Generate code for statements in loop body
				for (AST.Stmt stmt : doUntilStmt.stmts) {
					List<PDM.CodeInstr> stmtCode = stmt.accept(this, frame);
					if (stmtCode != null) {
						doUntilInstr.addAll(stmtCode);
					}
				}
				
				// Generate condition code
				List<PDM.CodeInstr> condCode = doUntilStmt.cond.accept(this, frame);
				if (condCode != null) {
					doUntilInstr.addAll(condCode);
				}
				
				// CJMP: if condition == 0 (false), continue loop; if condition != 0 (true), exit
				doUntilInstr.add(new PDM.NAME(startLabel, doUntilLoc));   // addr2 - continue loop (condition == 0)
				doUntilInstr.add(new PDM.NAME(endLabel, doUntilLoc));     // addr1 - exit loop (condition != 0)
				doUntilInstr.add(new PDM.CJMP(doUntilLoc));
				
				// End label
				doUntilInstr.add(new PDM.LABEL(endLabel, doUntilLoc));
				
				return doUntilInstr;
			}

			@Override
			public List<PDM.CodeInstr> visit(AST.BreakStmt breakStmt, Mem.Frame frame) {
				List<PDM.CodeInstr> breakInstr = new ArrayList<>();
				Report.Locatable breakLoc = attrAST.attrLoc.get(breakStmt);
				
				// For simple implementation: just generate error for now
				// In a real implementation, you'd need to track the current loop's end label
				throw new Report.Error(breakLoc, "Break statement not yet implemented");
				
				// If you want basic break support, you'd need to store the current loop's end label
				// and generate: NAME <end_label>, UJMP
			}

			// Do-While-Break
			// private String currentDoUntilEndLabel = null;
			// @Override
			// public List<PDM.CodeInstr> visit(AST.DoUntilStmt doUntilStmt, Mem.Frame frame) {
			// 	List<PDM.CodeInstr> doUntilInstr = new ArrayList<>();
			// 	Report.Locatable doUntilLoc = attrAST.attrLoc.get(doUntilStmt);
			...
			// 	// Set current do-until end label for break statements
			// 	String previousEndLabel = currentDoUntilEndLabel;
			// 	currentDoUntilEndLabel = endLabel;
			...
			// 	// Restore previous end label
			// 	currentDoUntilEndLabel = previousEndLabel;
				
			// 	return doUntilInstr;
			// }

			// // ADD this visit method for BreakStmt:
			...
			// 	// Check if we're inside a do-until loop
			// 	if (currentDoUntilEndLabel == null) {
			// 		throw new Report.Error(breakLoc, "Break statement can only be used inside do-until loops");
			// 	}
				
			// 	// Jump to current do-until loop's end label
			// 	breakInstr.add(new PDM.NAME(currentDoUntilEndLabel, breakLoc));
			// 	breakInstr.add(new PDM.UJMP(breakLoc));
